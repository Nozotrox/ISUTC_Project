...
public class ProductFrame extends JInternalFrame implements ActionListener {
    ...
    public ProductFrame(){
        ...
        String[][] data = null;
        // Column Names
        columnNames = new String[]{ "Codigo", "Armazém", "Stock Mínimo", "Quantidade", "Nome", "Fornecedor"};

        // Layout //

        JPanel north = new JPanel(new FlowLayout(FlowLayout.CENTER));
        JLabel label = new JLabel("Tela de Registo de Produto");
    ...
    public void write(){
        ...
        try {
            file_output = new FileOutputStream("Product.dat");
    ...
    private void read() {
        ...
        try {
            file_input = new FileInputStream("Product.dat");
            o_input = new ObjectInputStream(file_input);
            for(Object object:(Vector) o_input.readObject()){
                Vector ok= (Vector) object;
                String[] args=new String[]{""+ok.get(0),""+ok.get(1),""+ok.get(2), ""+ok.get(3),""+ok.get(4),""+ok.get(5)};
                model.addRow(args);
